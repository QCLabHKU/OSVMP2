...
def get_atom_dist(mol):
    coord_list = np.asarray([ico for ia, ico in mol._atom])*0.529177
    return scipy.spatial.distance.cdist(coord_list, coord_list)

def calc_cm5(mol, hirschfeld):
    
    #all matrices have the naming scheme matrix[k,k'] according to the paper
    distances = get_atom_dist(mol)
    atomicNumbers = mol.atom_charges()
