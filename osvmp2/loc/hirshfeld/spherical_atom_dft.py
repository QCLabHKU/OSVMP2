...
def _angular_momentum_for_each_ao(mol):
    ao_ang = np.zeros(mol.nao, dtype=int)
...
def main(atm, spin, basis, func):
    ...
    mol = gto.M(atom=atm+''' 0 0 0''', basis=basis, spin=spin_dict[atm])

    # Compute and save DM
    mol_avg = AtomSphericAverageDFT(mol, func)
