...
n_shm_core = shm_comm.size
n_node = nproc//n_shm_core
...
def lowdin(s):
    ...
    idx = e > 1e-15
    s12 = numpy.dot(v[:,idx]/numpy.sqrt(e[idx]), v[:,idx].conj().T)
    return s12

'''def lowdin(s):
    import scipy.linalg.interpolative as sli
    u, s_val, v = sli.svd(s)
    return numpy.dot(u/numpy.sqrt(s_val), v)'''
...
def orth_ao(mf_or_mol, method=ORTH_METHOD, pre_orth_ao=REF_BASIS, s=None):
    ...
    if method.lower() == 'lowdin':
        if pre_orth_ao is None:
            ...
            cal_s12 = True
            try:
                with h5py.File('int1e_ovlp_%d.tmp'%iproc, 'r') as f:
                    if f['s12'].shape[0] == s.shape[0]:
                        cal_s12 = False
                        c_orth = numpy.asarray(f['s12'])
            except OSError: pass
            if cal_s12:
                c_orth = lowdin(s)
                if iproc == 0:
                    with h5py.File('int1e_ovlp.tmp', 'w') as f:
                        f.create_dataset('s12', shape=c_orth.shape, dtype='f8')
                        f['s12'].write_direct(c_orth)
