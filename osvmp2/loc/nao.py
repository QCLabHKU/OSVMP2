...
def _nao_sub(mol, pre_occ, pre_nao, s=None):
    ...
    if rydbg_lst:
        ...
        c -= multi_dot([c1, c1.conj().T, s, c])
        s1 = multi_dot([c.conj().T, s, c])
        s_orth = orth.lowdin(s1)
        cnao[:,rydbg_lst] = lib.dot(c, s_orth)
    ...
    #print("nao_sub %.2f"%(time.time()-ttx))
    return cnao
