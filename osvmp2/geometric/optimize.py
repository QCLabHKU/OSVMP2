...
class Optimizer(object):
    ...
    def prepareFirstStep(self):
        ...
        if iproc == 0:
            msg = "\n--------------------------Step %d---------------------------------\n" %self.Iteration
            line = "--------------------------------------------------------------------\n"
            print(msg + "Energy = % .10f\nGradient RMS = %.3e\nGradient MAX = %.3e\n" % (self.E,rms_gradient, max_gradient) + line)
            # Initial history
        self.X_hist = [self.X]
    ...
    def step(self):
        ...
        '''if iproc == 0:
            if len(Eig) >= 6:
                logger.info("%d Hessian Eigenvalues: %.5e %.5e %.5e ... %.5e %.5e %.5e\n" % (self.Iteration,Eig[0],Eig[1],Eig[2],Eig[-3],Eig[-2],Eig[-1]))
            else:
                logger.info("%d Hessian Eigenvalues: " %self.Iteration + ' '.join("%.5e" % i for i in Eig) + '\n')'''
    ...
    def evaluateStep(self):
        ...
        # Print status
        if iproc == 0:
            print("\n--------------------------Step %d---------------------------" %self.Iteration)
            """msg += "E(change) = % .10f (%s%+.3e\x1b[0m) Quality = %s%.3f\x1b[0m\n" % (self.E, "\x1b[91m" if BadStep else ("\x1b[92m" if Converged_energy else "\x1b[0m"), self.E-self.Eprev, "\x1b[91m" if BadStep else "\x1b[0m", Quality)
            msg += "Grad%s = %s%.3e\x1b[0m/%s%.3e\x1b[0m (rms/max)\n" % ("_T" if self.IC.haveConstraints() else "", "\x1b[92m" if Converged_grms else "\x1b[0m", rms_gradient, "\x1b[92m" if Converged_gmax else "\x1b[0m", max_gradient)
            #msg = "Step %4i :" %self.Iteration
            msg += "Displace = %s%.3e\x1b[0m/%s%.3e\x1b[0m (rms/max)\n" % ("\x1b[92m" if Converged_drms else "\x1b[0m", rms_displacement, "\x1b[92m" if Converged_dmax else "\x1b[0m", max_displacement)
            line = "--------------------------------------------------------------------\n"
            logger.info(msg + "Trust = %.3e (%s)\n" % (self.trust, self.trustprint) + line)"""
            del_e = float(np.abs(self.E-self.Eprev))
            criteria = [
                ('%d* Energy change:'%self.Iteration, del_e, params.Convergence_energy),
                ('%d* Gradient RMS:'%self.Iteration, rms_gradient, params.Convergence_grms),
                ('%d* Gradient maximum:'%self.Iteration, max_gradient, params.Convergence_gmax)
            ]
            criteria.extend([
                ('%d* Displacement RMS:'%self.Iteration, rms_displacement, params.Convergence_drms),
                ('%d* Displacement maximum:'%self.Iteration, max_displacement, params.Convergence_dmax)
            ])
            ...
            for crit in criteria:
                ...
                k = 'OK' if result else 'no'
                msg += ('=>',k,)
                #msg = '* {} => {}'.format(msg, 'OK' if result else 'no')
                data_save.append(msg)
                #logger.info(msg)
                if not result:
                    all_matched = False
            lens = []
            for column in zip(*data_save):
                lens.append(max([len(x) for x in column]))
            format = "  ".join(["{:<" + str(x) + "}" for x in lens])
            for row_i in data_save:
                print(format.format(*row_i))
            if all_matched:
                print('%d* All criteria matched'%self.Iteration)
            print("-----------------------------------------------------------\n\n")
        ...
        if Converged_energy and Converged_grms and Converged_drms and Converged_gmax and Converged_dmax and self.conSatisfied:
            if iproc == 0:logger.info("Converged! =D\n")
            self.state = OPT_STATE.CONVERGED
            return

        if self.Iteration > params.maxiter:
            if iproc == 0:logger.info("Maximum iterations reached (%i); increase --maxiter for more\n" % params.maxiter)
            self.state = OPT_STATE.FAILED
            return
        if self.Iteration > 15 and ((Converged_energy and Converged_grms and Converged_gmax) or (Converged_grms and Converged_gmax and Converged_drms and Converged_dmax)):
            if iproc == 0:logger.info("Conditionally converged\n")
            self.state = OPT_STATE.CONVERGED
            return

        if params.qccnv and Converged_grms and (Converged_drms or Converged_energy) and self.conSatisfied:
            if iproc == 0:logger.info("Converged! (Q-Chem style criteria requires grms and either drms or energy)\n")
            self.state = OPT_STATE.CONVERGED
            return

        if params.molcnv and Converged_molpro_gmax and (Converged_molpro_dmax or Converged_energy) and self.conSatisfied:
            if iproc == 0:logger.info("Converged! (Molpro style criteria requires gmax and either dmax or energy)\nThis is approximate since convergence checks are done in cartesian coordinates.\n")
            self.state = OPT_STATE.CONVERGED
            return
...
def run_optimizer(**kwargs):
    ...
    if kwargs.get('logIni') is None:
        from osvmp2 import geometric
        import osvmp2.geometric.optimize
    ...
    print_msg()
    if iproc ==0:logger.info("Time elapsed since start of run_optimizer: %.3f seconds\n" % (time.time()-t0))
    return progress
