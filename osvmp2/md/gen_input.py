...
def read_xyz(xyz_file):
    with open(xyz_file, 'r') as f:
        ...
        for l in lines[2:2+natom]:
            coord += [float(i) for i in l.split()[1:]]
    return np.asarray(coord)

def get_SideLength(xyz_name, times=10):
    coord_list = read_xyz(xyz_name)
    #return max(atom_dist)
    max_xyz = np.max(coord_list, axis=0)
    min_xyz = np.min(coord_list, axis=0)
    return times*np.linalg.norm(max_xyz-min_xyz), (max_xyz+max_xyz)/2

def get_cen_atom(xyz_name, coord_center):
    coord_list = read_xyz(xyz_name)
    idx_list = range(len(coord_list))
    dist_list = []
    for ia, coi in enumerate(coord_list):
        dist_list.append([np.linalg.norm(coord_center-coi), ia])
    dist_list.sort()
...
class md_parameters():
    def __init__(self):
        ...
        self.len_side, coord_center = get_SideLength(self.xyz_name)
        '''self.chkfile_md = os.environ.get("chkfile_md", None)
        ...
        self.thermostat  = os.environ.get("thermostat ", 'pile_g')
        self.barostat = os.environ.get("barostat", 'isotropic')
        self.tau = os.environ.get("tau", 100)
        self.time_step = os.environ.get("time_step", 0.25)'''

        self.chkfile_md   = inputs["chkfile_md"]
        self.output_opt   = inputs["output_opt"]

        # Set up MD units
        self.time_unit    = inputs["time_unit"]
        self.temp_unit    = inputs["temp_unit"]
        self.potential_unit = inputs["potential_unit"]
        self.press_unit   = inputs["press_unit"]
        self.cell_units   = inputs["cell_units"]
        self.force_unit   = inputs["force_unit"]

        # Set up MD parameters
        self.verbose      = inputs["verbose"]
        self.stride       = inputs["stride"]
        self.nbeads       = inputs["nbeads"]
        self.port         = inputs["port"]
        self.total_steps  = inputs["total_steps"]
        self.seed         = inputs["seed"]
        self.temperature  = inputs["temperature"]
        self.pressure     = inputs["pressure"]
        self.if_fixatoms  = inputs["if_fixatoms"]
        self.fixatoms     = inputs["fixatoms"]
        self.fixcom       = inputs["fixcom"]
        self.dyn_mode     = inputs["dyn_mode"]
        self.temp_ensem   = inputs["temp_ensem"]
        self.thermostat   = inputs["thermostat"]
        self.barostat     = inputs["barostat"]
        self.tau          = inputs["tau"]
        ...
        if self.if_fixatoms and self.fixatoms is None:
            self.cen_atom = get_cen_atom(self.xyz_name, coord_center)
    ...
    def gen_input(self):
        ...
        if self.chkfile_md is not None:
        ...
        if self.dyn_mode == 'npt':
            text += "\t\t\t\t<barostat mode='%s'>\n\t\t\t\t\t<tau units='%s'> %s </tau>\n"%(self.barostat, self.time_unit, self.tau)
            text += "\t\t\t\t\t<thermostat mode='%s'>\n\t\t\t\t\t\t<tau units='%s'> %s </tau>\n\t\t\t\t\t</thermostat>\n\t\t\t\t</barostat>\n"%(self.thermostat , self.time_unit, self.tau)
        if self.dyn_mode == 'nvt' or self.dyn_mode == 'npt':
            text += "\t\t\t\t<thermostat mode='%s'>\n\t\t\t\t\t<tau units='%s'> %s </tau>\n\t\t\t\t</thermostat>\n"%(self.thermostat , self.time_unit, self.tau)
