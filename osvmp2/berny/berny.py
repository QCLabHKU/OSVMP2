...
n_node = nproc//shm_comm.size # number of nodes
...
class Berny(Generator):
    ...
    def send(self, energy_and_gradients):  # noqa: D102
        ...
        dq, dE, on_sphere = quadratic_step(
            self._n, dot(proj, s.interpolated.g), H_proj, s.weights, s.trust, log=log
        )
        ...
        self._converged = is_converged(
            self._n, dE, current.g, s.future.q - current.q, on_sphere, s.params, log=log
        )
...
def quadratic_step(iteration, g, H, w, trust, log=no_log):
    ...
    #if iproc == 0: log = print
    log('\n* Trust radius: {:.2}'.format(trust))
...
def is_converged(iteration, dE, forces, step, on_sphere, params, log=no_log):
    '''criteria = [
    ...
    criteria = [
        ('%d* Gradient RMS:'%iteration, Math.rms(forces), params['gradientrms']),
        ('%d* Gradient maximum:'%iteration, np.max(abs(forces)), params['gradientmax'])
    ]
    criteria.extend([
        ('%d* Displacement RMS:'%iteration, Math.rms(step), params['steprms']),
        ('%d* Displacement maximum:'%iteration, np.max(abs(step)), params['stepmax'])
    ])
    
    log("-----------------------------------------------------------")
    log('Convergence criteria:')
    log('%d* Predicted energy change: %.2E'%(iteration, dE))
    all_matched = True
    data_save = []
    for crit in criteria:
        if len(crit) > 2:
    ...
    for row_i in data_save:
        log(format.format(*row_i))
    if all_matched:
        log('%d* All criteria matched'%iteration)
    log("-----------------------------------------------------------")
    return all_matched