...
void locJacobiCuda(double *pija, double *ut, int nmo, int natom,
    ...
    double *h_aij = new double[blocksAtom];
    double *h_bij = new double[blocksAtom];
    size_t dAijSize = blocksAtom * sizeof(double);
    double *d_aij, *d_bij;
    cudaMalloc(&d_aij, dAijSize);
    cudaMalloc(&d_bij, dAijSize);
